          ...
          <div className="flex flex-col gap-0.5 sm:mt-2">
            {pathname !== "/incoming-payments" && (
              <Select
                ...
                value={
                  salesEmployees.find((emp) => emp.name === salesEmployee)
                    ?.code || ""
                }
            ...
            {pathname !== "/incoming-payments" ? (
              ...
              <Input
                ...
                value={remarks || ""}
                onChange={handleRemarksChange}
                readOnly={false} // Disable when status is not 'Open'}
                isEditable={isDocNumManuallyEntered}
                onIconClick={() => setIsOwnerModalOpen(true)}
                status={"Open"}
              />
            ...
            {pathname === "/incoming-payments" && (
              <Input
                ...
                onChange={handleJournalRemarksChange}
                readOnly={false}
                isEditable={isDocNumManuallyEntered}
                onIconClick={() => setIsOwnerModalOpen(true)}
                status={"Open"}
            ...
            {pathname === "/incoming-payments" && payOnAccount && (
              <Input
                ...
                onIconClick={() => setIsOwnerModalOpen(true)}
                status={status}
                style={{ visibility: isAccountSelected ? "hidden" : "visible" }}
              />
            )}
          ...
          <div className="flex flex-col gap-0.5 sm:mt-2">
            ...
            {pathname === "/incoming-payments" ? (
              <div className="flex items-center justify-between w-full">
                ...
                >
                  ...
                  <input
                    ...
                    onChange={handlePayOnAccountToggle}
                    className="h-3 w-3"
                  />

                  {/* Label */}
                  <label className="w-[10rem]">Payment on Account</label>

                  {/* Input Field */}
                  <input
                    type="text"
                    value={paymentOnAccount || ""}
                    disabled={!payOnAccount}
                    readOnly={isDocNumManuallyEntered || status !== "Open"} // Disable when status is not 'Open'}
                    onChange={handlePaymentOnAccountChange}
                    className={`border border-stone-400 focus:ring-2 focus:ring-yellow-600 focus:border-yellow-600 focus:border-0 focus:outline-none rounded-none w-[7.75rem] ${
                      isDocNumManuallyEntered
                        ? "bg-yellow-200"
                        : status === "Open"
                        ? "bg-white"
              ...
              <div className="flex items-center justify-between w-full">
                ...
                <div className="flex items-center">
                  <div
                    className={`input-with-currency relative ${
                      !discountValue ? "no-currency-label" : ""
                    }`}
                  >
                    <input
                      type="number"
            ...
            {pathname !== "/incoming-payments" && (
              <Input
                label={"Freight"}
                value={
                  freightTotal !== null && parseFloat(freightTotal) >= 0
                    ? `PKR ${Math.round(freightTotal * 100) / 100}`
                    : ctrlFEnterPressed
                    ? ""
                    : ""
                }
                ...
                onIconClick={() => setIsFreightModalOpen(true)}
                status={status}
                className={isDocNumManuallyEntered ? "bg-yellow-200" : ""}
                pathname={pathname}
                style={{ visibility: isAccountSelected ? "hidden" : "visible" }}
            ...
            <Input
              label={`${
                pathname !== "/incoming-payments" ? "Tax" : "Open Balance"
              }`}
              value={
                tax !== null && parseFloat(tax) !== 0.0
                  ? `PKR ${Math.round(tax * 100) / 100}`
                  : ctrlFEnterPressed
                  ? ""
                  : ""
              }
            ...
            {pathname !== "/incoming-payments" && (
              <Input
                label="Total"
                value={
                  total !== null && parseFloat(total) !== 0.0
                    ? `PKR ${total}`
                    : "PKR 0.000"
                }
                readOnly={isDocNumManuallyEntered || status !== "Open"} // Disable when status is not 'Open'}
